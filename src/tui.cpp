...
} g_consoleBuffers[kMaxBuffers];
static ConsoleBuffer& g_consoleBuffer = g_consoleBuffers[0];
static int g_consoleBufferIndex;
void PresentBuffer()
{
...
{
    PresentBuffer();
    g_consoleBufferIndex = (++g_consoleBufferIndex) % kMaxBuffers;
    g_consoleBuffer = g_consoleBuffers[g_consoleBufferIndex];
}
...
void TUI::Init(bool a_doubleBuffered)
{
    ...
    for (int i = 0; i < TUI_Platform::kMaxBuffers; ++i)
    {
        TUI_Platform::g_consoleBuffers[i].handle = CreateConsoleScreenBuffer(
            GENERIC_READ|GENERIC_WRITE,
            FILE_SHARE_READ|FILE_SHARE_WRITE,
            0,
            CONSOLE_TEXTMODE_BUFFER,
            0);
        TUI_Platform::Assert(TUI_Platform::g_consoleBuffers[i].handle != INVALID_HANDLE_VALUE, _T("Could not create console screen buffer: %ld\n"), GetLastError());
        SetConsoleMode(TUI_Platform::g_consoleBuffers[i].handle, ENABLE_PROCESSED_INPUT|DISABLE_NEWLINE_AUTO_RETURN);
        TUI_Platform::DisableCursor(TUI_Platform::g_consoleBuffers[i]);
    }
